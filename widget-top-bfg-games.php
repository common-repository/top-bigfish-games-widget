...
/*
Plugin Name: Widget Top BFG Games
Plugin URI: http://www.chocosnow.com/blog/widget-bfg-top-games/
Description: This simple plugin is a widget that displays a list of top casual games from BigFish on your widgetized sidebar.
Author: <a href="http://www.chocosnow.com/">ChocoSnow.com</a>, <a href="http://www.mrsnow.info/">Denis "Mr.Snow" Kozhukhov</a>
...
function widget_mrsnow_bfgtopgames_work ($before, $after)
{
	$options = (array) get_option ('widget_mrsnow_bfgtopgames');
	if ($options['channel']==null) 
	{
		$options['channel'] = str_replace ('.', '', str_replace ('_', '', str_replace ('-', '', $_SERVER['HTTP_HOST'])));
		update_option ('widget_mrsnow_bfgtopgames', $options);
	};
	...
	$above = $options['above'];
	$login = $options['login'];
	$logindef = $options['logindef'];
	$channel = $options['channel'];
	$showlink = $options['showlink'];
	if ($logindef != '2') $login = 'mrsnow';
	...
	if ($showlink == '1')
	{
		...
		$md5 = hash ("md5", $url);
		$options2 = $newoptions2 = get_option ('widget_mrsnow_bfgtopgames_'.$md5);
		...
		if ($options2['csurl'] == '') 
		{
			...
			else if ($variant == 1)
			{
				...
				if ($lang=='jp') 
					...
					$newoptions2['csurl'] = $csurl[$lang].GenerateIdFromTitle ($variantTitle).'/';
			...
			if ($options2 != $newoptions2) 
			{
				...
				update_option ('widget_mrsnow_bfgtopgames_'.$md5, $options2);
			};
		};	
...
function widget_mrsnow_bfgtopgames_control () 
{	
	$text['title'] = 'Widget Title:';
	$text['titled'] = 'Top BigFish Games';
	$text['gnum'] = 'Number of Games to Display:';
	$text['gtype'] = 'Games Type to Display:';
	$text['gtypep'] = 'PC Games';
	$text['gtypem'] = 'Mac Games (English sites only)';
	$text['gtypeo'] = 'Online Games';
	$text['glang'] = 'Games Language to Display:';
	$text['align'] = 'List Align:';
	$text['alignd'] = 'Theme default';
	$text['alignr'] = 'Right';
	$text['alignl'] = 'Left';
	$text['alignc'] = 'Center';
	$text['bullets'] = 'Show List Bullets';
	$text['pos'] = 'Show Games Position';
	$text['img'] = 'Show Games Images';
	$text['imgtype'] = 'Images Type:';
	$text['imgtypes'] = 'Small (60x40)';
	$text['imgtyped'] = 'Default (80x80)';
	$text['imgtypef'] = 'Feature (175x50)';
	$text['imgtypesf'] = 'Subfeature (175x50)';
	$text['shown'] = 'Show Games Title';
	$text['showt'] = 'Show Games Text';
	$text['titlepos'] = 'Title position:';
	$text['titleposti'] = 'Title Above Image';
	$text['titleposit'] = 'Image Above Title';
	$text['bfglogin'] = 'Your <a href="http://affiliates.bigfishgames.com/index.html?afcode=af6e89e0ae1a">BigFishGames affiliate</a> login:';
	$text['bfgloginnb'] = 'Please, use correct login. <br/> If you haven\'t one then <br/> use "mrsnow" login as default ';	
	$text['bfglogindef'] = 'Use default "mrsnow" login';
	$text['bfgchan'] = 'BigFishGames affiliate client channel:';
	$text['bfgchannb'] = 'Domain name used by default.<br/>Leave this field empty, to not use channels.<br/>Please use alpha numeric only.';
	$text['showlink'] = 'Embeed developer promo link';
	$text['copy'] = 'Copyright 2008-2011 by <a href="http://www.chocosnow.com/">ChocoSnow.com</a>';
	
	$text['error1'] = 'ERROR (options have been saved):<br/> Please, use correct affiliate login <br/> OR check to use the default login.<br/>Save options after corrections.';
	$text['error2'] = 'ERROR (options have been saved):<br/> Mac games available for <br/>English language only.<br/>Save options after corrections.';
	$text['error3'] = 'ERROR (options have been saved):<br/> Please select to display Games Title <br/>OR/AND Games Image.<br/>Save options after corrections.';
	$errortext = '';

	$options = $newoptions = get_option ('widget_mrsnow_bfgtopgames');
	...
	if ($_POST['bfgtopgames-submit']) 
	{
		...
		$newoptions['showname'] = ($_POST['bfgtopgames-showname'] ? '1' : '2');
		...
		$newoptions['logindef'] = ($_POST['bfgtopgames-logindef'] ? '1' : '2');
		...
		$newoptions['showlink'] = ($_POST['bfgtopgames-showlink'] ? '1' : '2');
	...
	$options['lang'] = $options['lang'] ? $options['lang'] : 'en';
	...
	$options['showname'] = $options['showname'] ? $options['showname'] : '1';
	...
	$options['logindef'] = $options['logindef'] ? $options['logindef'] : '1';
	if ($options['channel']==null) 
	{
		$options['channel'] = str_replace ('.', '', str_replace ('_', '', str_replace ('-', '', $_SERVER['HTTP_HOST'])));
		update_option ('widget_mrsnow_bfgtopgames', $options);
	...
	$options['showlink'] = $options['showlink'] ? $options['showlink'] : '1';
	
	$options['title'] = wp_specialchars ($options['title'], true);

	
	if($options['login'] == '' && $options['logindef'] != '1') $errortext = $text['error1'];
	if($options['type'] == 'mac' && $options['lang'] != 'en') $errortext = $text['error2'];
	if($options['showimage'] != '1' && $options['showname'] != '1') $errortext = $text['error3'];
...
?>
			...
			<label for="bfgtopgames-type" style="line-height:25px;display:block;">
				<?php _e($text['gtype'], 'widgets'); ?>
					<select style="width: 200px;" id="bfgtopgames-type" name="bfgtopgames-type">
						...
						<option value="mac"<?php if($options['type'] == 'mac') echo ' selected' ?>><?php _e($text['gtypem'], 'widgets'); ?></option>
						<option value="og"<?php if($options['type'] == 'og') echo ' selected' ?>><?php _e($text['gtypeo'], 'widgets'); ?></option>
					</select>
			</label>

			<label for="bfgtopgames-lang" style="line-height:25px;display:block;">
				<?php _e($text['glang'], 'widgets'); ?>
					<select style="width: 200px;" id="bfgtopgames-lang" name="bfgtopgames-lang">
						<option value="da"<?php if($options['lang'] == 'da') echo ' selected' ?>>Dansk (.dk)</option>
						<option value="de"<?php if($options['lang'] == 'de') echo ' selected' ?>>Deutsch (.de)</option>
						<option value="en"<?php if($options['lang'] == 'en') echo ' selected' ?>>English (.com, co.uk...)</option>
						<option value="es"<?php if($options['lang'] == 'es') echo ' selected' ?>>Español (.es)</option>
						<option value="fr"<?php if($options['lang'] == 'fr') echo ' selected' ?>>Français (.fr)</option>
						<option value="it"<?php if($options['lang'] == 'it') echo ' selected' ?>>Italiano (.it)</option>
						<option value="nl"<?php if($options['lang'] == 'nl') echo ' selected' ?>>Nederlands (.nl)</option>
						<option value="pt"<?php if($options['lang'] == 'pt') echo ' selected' ?>>Português (.com.br)</option>
						<option value="sv"<?php if($options['lang'] == 'sv') echo ' selected' ?>>Svenska (.se)</option>
						<option value="jp"<?php if($options['lang'] == 'jp') echo ' selected' ?>>日本語 (.jp)</option>
					</select>
			</label>
			...
			<label for="bfgtopgames-showname" style="line-height:25px;display:block;"><?php _e($text['shown'], 'widgets'); ?> <input type="checkbox" value="1" id="bfgtopgames-showname" name="bfgtopgames-showname"<?php echo ($options['showname']=='1') ? ' checked="checked"' : ''; ?> /></label>
			...
			<label for="bfgtopgames-logindef" style="line-height:25px;display:block;"><?php _e($text['bfglogindef'], 'widgets'); ?> <input type="checkbox" value="1" id="bfgtopgames-logindef" name="bfgtopgames-logindef"<?php echo ($options['logindef']=='1') ? ' checked="checked"' : ''; ?> /></label>
			...
			<hr/>
			<label for="bfgtopgames-showlink" style="line-height:25px;display:block;"><?php _e($text['showlink'], 'widgets'); ?> <input onclick="alert(1);" type="checkbox" value="1" id="bfgtopgames-showlink" name="bfgtopgames-showlink"<?php echo ($options['showlink']=='1') ? ' checked="checked"' : ''; ?> /></label>
			<hr/>
			
			<br/>
			<?php
			if($errortext != '') _e ('<font color="red">'.$errortext.'</font><br/>', 'widgets');
			?>
...
function widget_mrsnow_bfgtopgames_init () 
{
	...
	function widget_mrsnow_bfgtopgames ($args) 
	{
		extract ($args);
		//$start = mrsnow_microtime_float ();
		//echo $before_widget;
		echo "\n".'<!-- Widget Top BFG Games: START -->'."\n";
		echo widget_mrsnow_bfgtopgames_work ($before_title, $after_title);
		echo "\n".'<!-- Widget Top BFG Games: END -->'."\n";
		//echo $after_widget;
	...
	register_sidebar_widget (array ('Widget Top BFG Games', 'widgets'), 'widget_mrsnow_bfgtopgames');
	register_widget_control (array ('Widget Top BFG Games', 'widgets'), 'widget_mrsnow_bfgtopgames_control');
};

// Delay plugin execution to ensure Dynamic Sidebar has a chance to load first
add_action ('widgets_init', 'widget_mrsnow_bfgtopgames_init');
